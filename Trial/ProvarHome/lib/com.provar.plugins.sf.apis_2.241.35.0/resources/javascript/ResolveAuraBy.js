...
function resolveAuraBy(componentXPath, searchContext, multiple) {
    ...
    var flexiXpath="//div[((contains(@class,'windowViewMode-normal') and contains(@class,'oneContent')) or (contains(@class,'windowViewMode-maximized'))) and contains(@class,'active') and contains(@class,'lafPageHost')]";
    ...
    if(flexiNodes.length > 0){
        for(let i=0;i<flexiNodes.length;i++){
            ...
            while (dataAuraNode) {
	            	...
	            	if(dataAuraBy && isDetachedNode(dataAuraNode)){
...
function isDetachedNode(node) {
    if(node.parentNode?.getAttribute('data-aura-rendered-by')) {
        return false;
    }
    return true;
}
